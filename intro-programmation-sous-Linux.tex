...
\begin{frame}
  ...
  Les fichiers et répertoires ``n'existent pas'' et sont une abstraction fournie par le noyau du système d'exploitation. \textbf{\textcolor{red}{Les fichiers sont des séquences d'octets}} (souvent une dizaine, parfois des milliards).
  ...
  \item \texttt{/usr/bin/} et \texttt{/usr/sbin/}: executables usuels.
    \item \texttt{/etc/} : fichiers de configuration du système. Par
      ...
      \item \texttt{/dev/} : contient les pseudo-fichiers correspondant aux périphériques (``device'', par exemple \texttt{/dev/cdrom}).
  \end{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
  \item éviter les caractères bizarres, par exemple espace ou tabulation
  \item commencer et terminer un nom de fichier par une lettre ou chiffre ou blanc-souligné {\texttt{\_}}
