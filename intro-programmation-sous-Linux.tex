...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item \textbf{favoriser les échanges entre programmes} par des
      ...
      \item s'appuyer sur et \textbf{s'inspirer des commandes
        existantes} donc avec des noms \textit{en anglais}

    \item préférer la \textbf{lecture des données d'entrée sur
      l'entrée standard} (souvent notée \texttt{-} ou
      \texttt{/dev/stdin} dans les arguments du programme), et
      l'\textbf{écriture des résultats de sortie sur la sortie standard}
      (souvent passée comme \texttt{-} ou \texttt{/dev/stdout} en
      argument de programme)


  \item \textcolor{red}{\textbf{un programme qui fonctionne bien
      reste silencieux}}

%    et se termine en faisant un \textttt{exit(EXIT\_SUCCESS)} où
%    \texttt{EXIT\_SUCCESS} est 0.
%-
...
%-    \textttt{exit(EXIT\_FAILURE)} où \texttt{EXIT\_FAILURE} est 1,
%-    ou tout autre code d'erreur $>0$ et $<127$
%-
%-  \item tous les programmes acceptent les arguments \texttt{--help}
%-    (donnant un message d'aide) et \texttt{--version} (donnant leur
%-    version)
